...
const StatCard: React.FC<React.PropsWithChildren<{ title: string; icon: React.ElementType }>> = ({ title, icon: Icon, children }) => {
  return (
    <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4 flex flex-col h-full">
      ...
      <div className="text-lg font-semibold text-white mt-auto">{children}</div>
    </div>
  );
};
...
const ChartPlaceholder: React.FC<{ title: string; chartType: string; description?: string }> = ({ title, chartType, description }) => {
  return (
    <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4 h-full flex flex-col">
      <div className="flex items-center text-gray-400 mb-2">
        <Activity className="w-5 h-5 mr-2" /> {/* Generic icon */}
        <h3 className="text-sm font-medium">{title}</h3>
      </div>
  ...
  return (
    <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
      <div className="flex items-center mb-6">
        <User className="w-7 h-7 text-green-400 mr-3" />
        <h2 className="text-2xl font-semibold text-white">Personal Insights</h2>
      </div>
      <div className="grid grid-cols-1 gap-6">
        ...
        <div>
          <div className="flex items-center text-gray-400 mb-2">
            ...
            <h3 className="text-sm font-medium">Retention Score per Category</h3>
          </div>
        ...
        <div>
          <div className="flex items-center text-gray-400 mb-2">
            ...
            <h3 className="text-sm font-medium">Top Words You Struggle With</h3>
          </div>
          <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
            {stats.topStrugglingWords.length > 0 ? (
              ...
              <p className="text-sm text-gray-500">No specific struggling words identified yet.</p>
            )}
          </div>
        </div>
        <div>
          <div className="flex items-center justify-between text-gray-400 mb-2">
            <div className="flex items-center">
              ...
              <h3 className="text-sm font-medium">Custom Weekly Goals</h3>
            </div>
          ...
          <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4 space-y-2">
            {stats.customGoals.map((goal, index) => (
              <div key={index} className="flex items-center justify-between text-sm">
                <div className="flex items-center">
                  <input type="checkbox" checked={goal.completed} readOnly className="form-checkbox h-4 w-4 text-blue-500 bg-gray-700 border-gray-600 rounded mr-2 focus:ring-blue-600" />
